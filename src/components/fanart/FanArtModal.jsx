  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
      ...
      <motion.div
        ...
        className="relative w-full max-w-5xl max-h-[90vh] flex flex-col md:flex-row bg-[#0a0a0a] border border-white/10 rounded-2xl shadow-2xl overflow-hidden"
      >
        
        {/* Close Button (Floating) */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 p-2 bg-black/50 hover:bg-white/20 text-white rounded-full backdrop-blur-md transition-all border border-white/10"
        >
          <FaTimes className="text-lg" />
        </button>

        {/* 3. Image Section (Main Focus) */}
        <div className="flex-1 bg-black flex items-center justify-center p-4 md:p-8 relative overflow-hidden group">
            
            {/* Ambient Background Glow based on image context */}
            <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 to-purple-500/10 opacity-50 pointer-events-none" />

            <img
                src={art.image_url}
                alt="Fanart"
                className="relative z-10 max-h-[50vh] md:max-h-[80vh] w-auto object-contain shadow-[0_0_50px_rgba(0,0,0,0.5)] rounded-md"
            />
        </div>
